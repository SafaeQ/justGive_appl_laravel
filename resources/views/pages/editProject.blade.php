                    ...
                    <div>
                        <div class="container" style="position: relative; z-index: 1;">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="card1">
                                        <form method="POST" action={{ route('editProject',$project->id) }} class="box" enctype="multipart/form-data" style="width: 500px; padding: 40px;position: absolute;top: 50%;left: 50%; background: #191919;text-align: center;transition: 0.25s; margin-top: 100px; border-radius: 20px; box-shadow: -1px 4px 14px 7px rgb(0 0 0 / 40%)">
                                            @csrf
                                            <h4>Edit Project</h4>
                                            ...
                                            @enderror
                                            <input type="text" name="budget" placeholder="Budget" value="{{ $project->budget }}">
                                            @error("budget")
                                                <p class="text-danger">{{ $message}}</p>
                                            @enderror
                                            <textarea type="text" name="description" placeholder="Description"style="" value="">{{ $project->description }}</textarea>
                                            @error("description")
                                                <p class="text-danger">{{ $message}}</p>
                                            @enderror
                                            ...
                                            @error("image")
                                                <p class="text-danger">{{ $message}}</p>
                                            @enderror --}}
                                            <select name="category" id="cars" class="btn text-white text-center dropdown-toggle" style="width:250px;background-color: #75D3F5">
                                                @foreach ($categories as $category)
                                                <option value="{{ $category->id }}" {{ $category->id == $project->category_id ?"selected":''}}>{{ $category->name }}</option>
                                                @endforeach
                                                {{-- <option value="{{ $project->category_id }}">Health</option>
                                                <option value="{{ $project->category_id }}">Social</option> --}}
                                            </select>
                                            @error("category")
                                                <p class="text-danger">{{ $message}}</p>
                                            @enderror
                                            <input type="submit" name="" href="{{ route('account',['user']) }}" value="Edit project" href="#">
                                        </form>
                                    </div>
                                </div>
                            </div>
