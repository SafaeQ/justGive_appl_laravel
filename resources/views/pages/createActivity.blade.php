                    ...
                    <div>
                        <div class="container" style="position: relative; z-index: 1;">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="card1">
                                        <form method="POST" action="{{ route('createActivity') }}"class="box" enctype="multipart/form-data" style="width: 500px; padding: 40px;position: absolute;top: 50%;left: 50%; background: #191919;text-align: center;transition: 0.25s; margin-top: 100px; border-radius: 20px; box-shadow: -1px 4px 14px 7px rgb(0 0 0 / 40%)">
                                            @csrf
                                            <h4>Create Project's Activity</h4>
                                            ...
                                            @error("date")
                                                <p class="text-danger">{{ $message}}</p>
                                            @enderror
                                            <textarea type="text" name="description" placeholder="Description"style=""></textarea>
                                            @error("description")
                                                <p class="text-danger">{{ $message}}</p>
                                            @enderror
                                            ...
                                            <select name="projects_id" id="cars" class="btn text-white text-center dropdown-toggle" style="width:250px;background-color: #75D3F5">
                                                <option value="" selected disabled>project</option>
                                                @foreach ($projects as $project)
                                                <option value="{{ $project->id }}" {{-- $project->id==$project->project_id?"selected":'' --}}>{{ $project->name }}</option>
                                                @endforeach
                                            </select>
                                            @error("image")
                                                <p class="text-danger">{{ $message}}</p>
                                            @enderror
                                            <input type="submit" name="" value="Create Activity" href="#">
                                        </form>
                                    </div>
                                </div>
                            </div>
